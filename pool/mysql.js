...
// 创建 mysql 连接池并配置参数（线上数据库）
const mysqlConnection = mysql.createConnection({
    host: "rm-bp1571h7kq93656x9xo.mysql.rds.aliyuncs.com",    // 数据库主机外网地址
    // host: "127.0.0.1",    // 主机地址
    port: 3306,                 // 端口
    user: "root",               // 数据库访问账号
    password: "gaolong!123",           // 数据库访问密码
    database: "uni_app",           // 要访问的数据库
    charset: "UTF8_GENERAL_CI", // 字符编码 ( 必须大写 )
    typeCast: true,             // 是否把结果值转换为原生的 javascript 类型
    supportBigNumbers: true,    // 处理大数字 (bigint, decimal), 需要开启 ( 结合 bigNumberStrings 使用 )
    bigNumberStrings: true,     // 大数字 (bigint, decimal) 值转换为javascript字符对象串
    multipleStatements: false,  // 允许每个mysql语句有多条查询, 为防止sql注入不开启
    // stringifyObjects: true //  是否序列化数据
    // connectTimeout: 30000,     // 数据库连接超时时间, 默认无超时
})
...
mysqlConnection.connect((err) => {
    if (err) {
        console.log("【mysql数据库连接获取失败】",err);
    } else {
